...
import { Archive, Database, FileUp, Loader2, Pencil, Plus, X } from "lucide-react";
...
export function AdminCrud({ section, table, initialRecords, createEnabled }: { section: string; table: string; initialRecords: AdminRecord[]; createEnabled: boolean }) {
  ...
  return <><div className="crud-actions">{createEnabled && <button className="button button-primary" type="button" onClick={startCreate}><Plus size={17} /> Novo registro</button>}</div>{initialRecords.length ? <div className="admin-record-list">{initialRecords.map((item, index) => <article key={String(item.id ?? index)}><div><strong>{String(item[config.titleKey] ?? `Registro ${index + 1}`)}</strong><p>{String(item.summary ?? item.excerpt ?? item.requester_name ?? item.status ?? "Sem descrição")}</p></div><div><span className="status-badge status-informational">{String(item.status ?? "cadastrado")}</span><button type="button" onClick={() => startEdit(item)}><Pencil size={15} /> Editar</button>{config.archive && item.status !== "archived" && <button type="button" onClick={() => void archive(item)} disabled={saving}><Archive size={15} /> Arquivar</button>}</div></article>)}</div> : <div className="empty-admin"><Database size={28} /><strong>Nenhum registro neste módulo</strong><p>Use “Novo registro” para iniciar o conteúdo.</p></div>}
