...
export function AdminLoginForm({ authorizedEmail }: { authorizedEmail: string }) {
  ...
  return <div className="admin-login-card"><span className="admin-login-icon"><KeyRound /></span><span className="eyebrow">Acesso restrito</span><h1>Painel Sindicomar</h1><p>O acesso é permitido somente ao administrador previamente autorizado. Clique abaixo para receber o link seguro.</p><button className="button button-primary admin-login-submit" type="button" onClick={login} disabled={loading}>{loading ? <><Loader2 className="spin" size={18} /> Enviando…</> : "Enviar link seguro"}</button>{message && <p className="admin-login-message" role="status">{message}</p>}<small>Depois do link, será exigido o segundo fator de autenticação.</small></div>;
}
